...
const helpData: HelpDataProps[] = [
    ...
    text: (
      <>
        ...
        <Typography variant="body1">
          После ввода более трёх символов в форму поиска адресная книга отобразит все строки с найденными значениями вне зависимости от
          того, в каком из вышеперечисленных столбцов найдена информация. Например, при вводе цифр 1112 отобразится список системных
          администраторов с телефоном 1112
        </Typography>
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <Typography variant="body1">
          При нажатии на строку адресной книги в появившемся всплывающем окне отобразится подробная информация о сотруднике. Здесь активными
          являются строки: «Компания», «Департамент», «Должность», «Отдел», «Руководитель» и e-mail. Например, при нажатии на «Департамент»
          отобразятся все сотрудники данного департамента. При нажатии на строку «Руководитель» откроется карточка руководителя сотрудника.
          При нажатии на e-mail откроется форма отправки почты с заполненным адресатом.
        </Typography>
        <Typography variant="body1">
  ...
  },
  {
    ...
    text: (
      <>
        <Typography variant="body1">
          Для настройки отображения колонок адресной книги используйте значок <Settings style={{ opacity: 0.6 }} fontSize="small" /> в
          правом верхнем углу. В появившемся всплывающем окне отметьте галочками какую информацию Вам нужно отобразить. Затем нажмите кнопку
          «Применить». Эти настройки запоминаются в профиле пользователя и не сбиваются после выхода из адресной книги.
        </Typography>
        <Typography variant="body1">Для настройки отображения колонок в значения по умолчанию используйте кнопку «Сбросить».</Typography>
      </>
    ),
  },
];
            ...
            {Math.abs(step - idx) <= 2 ? (
              <Paper square elevation={0} className={classes.content}>
                <Box className={classes.imageBox}>
                  {typeof current.image === 'string' ? <img src={current.image} alt="help" /> : <BlockIcon />}
                </Box>
